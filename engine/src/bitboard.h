...
inline uint64_t BETWEEN_BB_SAFE(int from, int to) {
  if (!is_valid_square(from) || !is_valid_square(to)) return 0ULL;
  return BetweenBBs[static_cast<size_t>(from)][static_cast<size_t>(to)];
}
